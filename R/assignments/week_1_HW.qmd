...
islands_vector <- unique(penguins$island)
```
...
```{r}
penguins %>% unite(col = species_island, species, island, sep = "_")
```
...
```{r}
# Your code here
new_dream_penguins <- dream_penguins %>% select(species, bill_length_mm, bill_depth_mm)
```
...
penguins %>%
  ...
  group_by(species) %>%
  summarise(avg_bill_length_mm = mean(penguins$bill_length_mm, na.rm = TRUE))
...
```{r}
# Your code here
penguins_in <- penguins %>% mutate(flipper_length_in = flipper_length_mm / 25.4)
```
...
ggplot(data = penguins, mapping = aes(x = bill_depth_mm, y = bill_length_mm)) +
  geom_point() +
  ggtitle("Penguin Bill Dimension")
```
...
# Your code here
penguins <- penguins %>%
  mutate(is_heavy = body_mass_g > 4500)
```
...
penguins %>%
  ...
  ggplot(aes(x = year, y = avg_mass)) + geom_line() + geom_point()
```
